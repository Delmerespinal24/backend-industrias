...
const addMachine = async (req, res) => {
    ...
    try {
        ...
        //Devolver el id del producto agregado para la imagen
        var response = await pool.query('SELECT LAST_INSERT_ID() AS idMaquina')


        var resultado = {
            status: status,
            message: message,
            idMaquina: response[0].idMaquina
        }
...
const deleteMachine = async (req, res) => {
    ...
    try {
        ...
        if (rows.length == 0) {
            ...
            let images = await connection.query("Select image_1, image_2, image_3 FROM maquina WHERE idMaquina = ?", [idMaquina]);
            //Elimina todas las imagenes que contiene el producto del servidor
            const files = []
            console.log(images[0])
            
            if(images[0].image_1 != "" && images[0].image_1){
                files.push("src/ImagenesMaquinaria/" + images[0].image_1)
            }
            if(images[0].image_2 != "" && images[0].image_2){
                files.push("src/ImagenesMaquinaria/" + images[0].image_2)
            }
            if(images[0].image_3 != "" && images[0].image_3){
                files.push("src/ImagenesMaquinaria/" + images[0].image_3)
            }
